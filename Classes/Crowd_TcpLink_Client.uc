...
{
    super.PostBeginPlay();
    class'Crowd_CrowdControl_Gamemod'.static.DebugLog("[CrowdControl] Resolving: ");
    class'Crowd_CrowdControl_Gamemod'.static.DebugLog(TargetHost);
    resolve(TargetHost);
}

event Resolved( IpAddr Addr )
{
    class'Crowd_CrowdControl_Gamemod'.static.DebugLog("[CrowdControl] " $ TargetHost $ " resolved to " $ IpAddrToString(Addr));
    if(IpAddrToString(Addr) == "0.0.0.0:0")
    {
        class'Crowd_CrowdControl_Gamemod'.static.DebugLog("Could not connect to Crowd Control. Please read Mod Description.", true);
        Destroy();
        return;
    }
    ...
    class'Crowd_CrowdControl_Gamemod'.static.DebugLog("[CrowdControl] Bound to port: "$ BindPort());

    if (!Open(Addr))
    {
        class'Crowd_CrowdControl_Gamemod'.static.DebugLog("[CrowdControl] Open failed");
    }
}

event ResolveFailed()
{
    class'Crowd_CrowdControl_Gamemod'.static.DebugLog("[CrowdControl] Unable to resolve " $ TargetHost);
    class'Crowd_CrowdControl_Gamemod'.static.DebugLog("Could not connect to Crowd Control. Please read Mod Description.", true);
    Destroy();
}
...
event Closed()
{
    ...
    class'Crowd_CrowdControl_Gamemod'.static.DebugLog("[CrowdControl] event closed");
}
...
event ReceivedBinary(int Count, byte B[255])
{
    ...
    //class'Crowd_CrowdControl_Gamemod'.static.DebugLog("[CrowdControl] ReceivedText:: "$ReceivedText);
    ParsedJson = class'JsonObject'.static.DecodeJson(ReceivedText);
    if (ParsedJson == None) return;
    //class'Crowd_CrowdControl_Gamemod'.static.DebugLog("[CrowdControl] Received command: "$ParsedJson.GetStringValue("code"));
    responseCode = class'Crowd_CrowdControl_Gamemod'.static.GetGameMod().ProcessCode(ParsedJson.GetStringValue("code"), ParsedJson.GetIntValue("id"), ParsedJson.HasKey("duration") ? ParsedJson.GetIntValue("duration") : -1, timeRemaining);
    ...
    //class'Crowd_CrowdControl_Gamemod'.static.DebugLog("[CrowdControl] Sending command: "$responseText);
